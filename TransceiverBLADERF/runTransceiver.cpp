...
static std::string init_devaddr()
{
  ...
  try {
    addr = gConfig.getNum("UMTS.Radio.BLADERF.DeviceAddress");
  } catch (ConfigurationTableKeyNotFound e) {
    addr = "";
  }
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "** Searching for bladeRF device " << devaddr << std::endl;
  bladeRF = new bladeRFDevice(DEVICE_RATE);
  found = bladeRF->open(devaddr, extref);
#if 0
